...
function Term({ power, coefficient }) {
    ...
    if (power === 0) {
        // constant -> render only coefficient
        return <React.Fragment> {renderedCoefficient}</React.Fragment>
    } else if (power === 1) {
        // linear -> render just 'x'
        return <React.Fragment> {renderedCoefficient}x</React.Fragment>
    } else {
        // higher powers --> render superscript exponent
        return (
            <React.Fragment>
                {' '}
                {renderedCoefficient}x<sup>{power}</sup>
            </React.Fragment>
        )
    }
}
...
function Polynomial({ data }) {
    return (
        <React.Fragment>
            <span>f(x) = </span>
            <CubicTerm coefficient={data.get(3)} />{' '}
            {needsConcatination(3, data) && data.get(2) > 0 ? <Plus /> : null}
            <QuadraticTerm coefficient={data.get(2)} />{' '}
            {needsConcatination(2, data) && data.get(1) > 0 ? <Plus /> : null}
            <LinearTerm coefficient={data.get(1)} />{' '}
            {needsConcatination(1, data) && data.get(0) > 0 ? <Plus /> : null}
